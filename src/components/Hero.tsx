...
const slides: HeroSlide[] = [
  {
    ...
    labels: [
      { text: "CS", position: "left-[10%] top-[23%]" },
      { text: "COMMUNITY", position: "right-[7%] top-[31%]" },
      { text: "BUILD", position: "bottom-[18%] left-[13%]" },
    ],
    cards: [
  ...
  },
  {
    ...
    labels: [
      { text: "SOFTWARE", position: "left-[7%] top-[26%]" },
      { text: "BUILD", position: "right-[12%] top-[22%]" },
      { text: "CODE", position: "bottom-[17%] right-[12%]" },
    ],
    cards: [
  ...
  },
  {
    ...
    labels: [
      { text: "CYBER", position: "left-[9%] top-[25%]" },
      { text: "DATA", position: "right-[10%] top-[29%]" },
      { text: "CAREER", position: "bottom-[17%] left-[13%]" },
    ],
    cards: [
...
const INTERACTION_PAUSE_DELAY = 7_000;
...
export default function Hero() {
  ...
  useEffect(() => {
    if (prefersReducedMotion || isHovered || isUserPaused || !isPageVisible) {
      return;
    }
  ...
  }, [activeSlide, isHovered, isPageVisible, isUserPaused, prefersReducedMotion]);
  ...
  const handlePointerEnd = (event: React.PointerEvent<HTMLDivElement>) => {
    if (swipeStartX.current === null || event.pointerType !== "touch") {
      return;
    }
      ...
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-12 px-5 pb-18 pt-12 sm:px-6 sm:pb-22 sm:pt-16 lg:grid-cols-[1.02fr_0.98fr] lg:gap-14 lg:px-8 lg:pb-24 lg:pt-18">
        <div className="max-w-2xl">
          ...
          <p className="hero-fade-up-delay-2 mt-6 max-w-xl text-base leading-7 text-slate-600 sm:mt-7 sm:text-lg sm:leading-8">
            A student-led community at Nile University where computing students learn,
            build, connect, and create opportunities together.
          </p>
        ...
        <div className="hero-fade-up-delay-2 relative mx-auto w-full max-w-xl lg:max-w-none">
          <div
            className="hero-card relative mx-auto aspect-[0.94] w-full max-w-[520px] overflow-hidden rounded-[1.75rem] border border-[#dce5f8] bg-gradient-to-br from-[#f8fbff] via-white to-[#eaf0ff] p-4 shadow-[0_30px_80px_rgba(39,65,147,0.13)] sm:rounded-[2rem] sm:p-7 lg:max-w-[560px]"
            role="region"
            ...
            onFocusCapture={pauseAutoplayAfterInteraction}
            style={{ touchAction: "pan-y" }}
          >
            <div className="absolute inset-0 bg-[linear-gradient(rgba(39,65,147,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(39,65,147,0.06)_1px,transparent_1px)] bg-[size:34px_34px]" />
            <div className="absolute -right-20 -top-20 h-60 w-60 rounded-full bg-[#d8e5ff] blur-3xl" />
            <div className="absolute -bottom-20 -left-16 h-56 w-56 rounded-full bg-[#e8edff] blur-3xl" />

            <div className="relative flex h-full flex-col justify-between">
              <div className="flex items-start justify-between gap-3">
                <div key={activeSlide} className="hero-slide-content">
                ...
                <div
                  className="flex shrink-0 items-center gap-0.5 rounded-full border border-white/90 bg-white/80 p-1 shadow-sm backdrop-blur"
                  ...
                  {slides.map((item, index) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={`Show community slide ${index + 1}: ${item.eyebrow}`}
                        aria-pressed={isActive}
                        className={`group inline-flex h-6 w-6 items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-[#274193] focus:ring-offset-2 ${
                          isActive ? "bg-[#edf3ff]" : "hover:bg-slate-50"
                        }`}
                      >
                        <span
                          className={`rounded-full transition-[width,background-color] duration-200 motion-reduce:transition-none ${
                            isActive ? "h-2.5 w-5 bg-[#274193] shadow-[0_1px_3px_rgba(39,65,147,0.28)]" : "h-1.5 w-1.5 bg-[#aec4ee] group-hover:bg-[#60a5fa]"
                          }`}
                          aria-hidden="true"
                        />
              ...
              <div key={`visual-${activeSlide}`} className="hero-slide-content relative flex flex-1 items-center justify-center py-4 sm:py-6">
                <div className="absolute h-48 w-48 rounded-full border border-[#c8d7f5] sm:h-64 sm:w-64" />
                <div className="absolute h-36 w-36 rounded-full border border-dashed border-[#b7c8ee] sm:h-48 sm:w-48" />
                <div className="absolute h-24 w-24 rounded-full border border-[#d5e0f6] sm:h-32 sm:w-32" />
                ...
                {slide.labels.map((label) => (
                  <span
                    key={label.text}
                    className={`absolute hidden rounded-lg border border-white bg-white/90 px-2.5 py-1.5 text-[9px] font-bold tracking-[0.08em] text-[#274193] shadow-[0_8px_24px_rgba(39,65,147,0.1)] backdrop-blur md:block ${label.position}`}
                  >
                    {label.text}
                  </span>
                ))}
              ...
              <div key={`cards-${activeSlide}`} className="hero-slide-content grid grid-cols-3 gap-2 sm:gap-3">
                {slide.cards.map((card) => (
                  <div key={card.title} className="rounded-xl border border-white/80 bg-white/75 p-2.5 shadow-sm backdrop-blur sm:rounded-2xl sm:p-3">
                    <p className="text-[10px] font-bold text-[#0d1733] sm:text-xs">{card.title}</p>
                    <p className="mt-1 hidden text-[10px] leading-4 text-slate-500 sm:block">{card.description}</p>
                  </div>
                ))}
              </div>
            </div>
