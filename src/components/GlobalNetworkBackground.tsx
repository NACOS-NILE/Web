...
export default function GlobalNetworkBackground() {
  ...
  // Track mouse internally to avoid passing props
  ...
  useEffect(() => {
    ...
    for (let c = 0; c < cols; c++) {
      for (let r = 0; r < rows; r++) {
        // 70% chance to have a node at this grid intersection (denser)
        if (Math.random() > 0.3) {
    ...
    // 2. Generate PCB Traces (orthogonal + 45deg)
    for (const node of nodes) {
      const c = node.col;
      const r = node.row;
      
      // Connect Right (50% chance)
    ...
    const render = (time: number) => {
      ...
      for (const edge of edges) {
        ...
        // Spotlight Math (compensated for drift)
