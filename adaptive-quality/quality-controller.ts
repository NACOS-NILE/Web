...
export interface QualityControllerOptions<T extends TierResources> {
  /** Defaults to `auto`, which begins High unless initialDowngrade is set. */
  readonly initialPreference?: QualityPreference;
  /** Apply an already-known constraint before allocating the first scene. */
  readonly initialDowngrade?: DowngradeReason;
...
export function createQualityController<T extends TierResources>(
  ...
  let requestedTier: QualityTier = preference === 'low' || (preference === 'auto' && options.initialDowngrade) ? 'low' : 'high';
  let requestedReason: QualityReason = preference === 'auto' ? options.initialDowngrade ?? 'initial' : 'forced';
