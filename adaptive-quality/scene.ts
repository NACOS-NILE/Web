...
import daylightWgsl from './daylight.wgsl';
 ...
 */
export function createScene(gpu: Gpu, output: Output, tier: QualityTier, daylight = false): Scene {
  const settings = { ...TIER_SETTINGS[tier], bloom: !daylight && TIER_SETTINGS[tier].bloom };
  const owned: Target[] = [];
  ...
  try {
    ...
    const field = effect(gpu, daylight ? daylightWgsl : fieldWgsl, {
